...
impl Rasterizer {
    ...
    pub fn draw_tri(
        ...
        let mut max_x =
            (max_f32(&xs).ceil() as usize).clamp(0, self.real_dims.0 - 1);
        let mut max_y =
            (max_f32(&ys).ceil() as usize).clamp(0, self.real_dims.1 - 1);

        let outer_tile_indx = TILE_SIZES
            .iter()
            .position(|&x| 2 * x >= max_x - min_x && 2 * x >= max_y - min_y)
            .unwrap_or(OUTER_TILE_INDEX);
        let align_to = TILE_SIZES[outer_tile_indx];

        min_x = (min_x / align_to) * align_to;
        min_y = (min_y / align_to) * align_to;
        max_x = max_x.next_multiple_of(align_to);
        max_y = max_y.next_multiple_of(align_to);
